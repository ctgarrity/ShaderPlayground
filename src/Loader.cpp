...
glm::mat4 to_glm(const fastgltf::math::fmat4x4& m) {
    ...
    for (int c = 0; c < 4; ++c)
        for (int r = 0; r < 4; ++r)
            out[c][r] = m[c][r]; // both are column-major
    return out;
}
...
std::optional<std::shared_ptr<LoadedGLTF>> load_gltf(Renderer* renderer, const std::string_view file_path) {
    ...
    constexpr auto gltf_options = fastgltf::Options::DontRequireValidAssetMember | fastgltf::Options::AllowDouble | fastgltf::Options::LoadExternalBuffers; // fastgltf::Options::LoadGLBBuffers deprecated
    ...
    file.descriptor_pool.init(renderer->m_vkb_device.device, gltf.materials.size(), sizes);

    // load samplers
    for (fastgltf::Sampler& sampler : gltf.samplers) {
        VkSamplerCreateInfo sampler_info = { .sType = VK_STRUCTURE_TYPE_SAMPLER_CREATE_INFO, .pNext = nullptr};
        ...
        vkCreateSampler(renderer->m_vkb_device.device, &sampler_info, nullptr, &new_sampler);
    ...
    for (fastgltf::Image& image : gltf.images) {
        ...
        if (img.has_value()) {
            ...
            images.push_back(renderer->m_error_checkerboard_image);
            std::cout << "gltf failed to load texture " << image.name << std::endl;
        }
    }

    // create buffer to hold the material data
    file.material_data_buffer = renderer->create_buffer(sizeof(GLTFMetallicRoughness::MaterialConstants) * gltf.materials.size(),VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VMA_MEMORY_USAGE_CPU_TO_GPU);
    int data_index = 0;
    auto sceneMaterialConstants = (GLTFMetallicRoughness::MaterialConstants*)file.material_data_buffer.info.pMappedData;

    for (fastgltf::Material& material : gltf.materials) {
        std::shared_ptr<GLTFMaterial> new_material = std::make_shared<GLTFMaterial>();
        materials.push_back(new_material);
        file.materials[material.name.c_str()] = new_material;
        ...
        material_resources.color_image = renderer->m_white_image;
        material_resources.color_sampler = renderer->m_default_sampler_linear;
        material_resources.metal_rough_image = renderer->m_white_image;
        material_resources.metal_rough_sampler = renderer->m_default_sampler_linear;
        ...
        // build material
        new_material->data = renderer->m_metal_rough_material.write_material(renderer->m_vkb_device.device, pass_type, material_resources, file.descriptor_pool);
    ...
    for (fastgltf::Mesh& mesh : gltf.meshes) {
        ...
        new_mesh->mesh_buffers = std::make_shared<GPUMeshBuffers>(renderer->upload_mesh(indices, vertices));
    }
    ...
    for (fastgltf::Node& node : gltf.nodes) {
        ...
        // From ChatGPT replacing Viktor's code
        fastgltf::math::fmat4x4 fmat = fastgltf::getTransformMatrix(node);
        newNode->local_transform = to_glm(fmat);
    }
...
void LoadedGLTF::Draw(const glm::mat4 &top_matrix, DrawContext &draw_context) {
...
void LoadedGLTF::clear_all() {
    VkDevice dv = creator->m_vkb_device.device;
    ...
    for (auto& [k, v] : images) {
        ...
        if (v.image == creator->m_error_checkerboard_image.image) {
        ...
        fastgltf::visitor {
            ...
            [&](fastgltf::sources::URI& filePath) {
                ...
                if (data) {
                    ...
                    new_image = renderer->create_image(data, imagesize, VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_USAGE_SAMPLED_BIT,false);

                    stbi_image_free(data);
                }
            },
            [&](fastgltf::sources::Vector& vector) {
                auto src = reinterpret_cast<const stbi_uc*>(vector.bytes.data()); // From ChatGPT
                unsigned char* data = stbi_load_from_memory(src, static_cast<int>(vector.bytes.size()),
            ...
            [&](fastgltf::sources::BufferView& view) {
                ...
                std::visit(fastgltf::visitor { // We only care about VectorWithMime here, because we
                               ...
                               [](auto& arg) {},
                               [&](fastgltf::sources::Vector& vector) {
                                   const std::byte* base = vector.bytes.data();
                                   auto src = reinterpret_cast<const stbi_uc*>(base + static_cast<size_t>(bufferView.byteOffset));
                                   int len = static_cast<int>(bufferView.byteLength);
                                   unsigned char* data = stbi_load_from_memory(src, len, &width, &height, &nr_channels, 4);
                                   if (data) {
                                       VkExtent3D imagesize;
                                       imagesize.width = width;
                                       imagesize.height = height;
                                       ...
                                       new_image = renderer->create_image(data, imagesize, VK_FORMAT_R8G8B8A8_UNORM,
                    ...
                    [&](fastgltf::sources::Array& array) {
                                    const std::byte* base = array.bytes.data();
                                    auto src = reinterpret_cast<const stbi_uc*>(base + static_cast<size_t>(bufferView.byteOffset));
                                    int len = static_cast<int>(bufferView.byteLength);
                                    unsigned char* data = stbi_load_from_memory(src, len, &width, &height, &nr_channels, 4);
                                    if (data) {
